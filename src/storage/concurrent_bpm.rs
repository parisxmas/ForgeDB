...
//! Only same-page access serializes. The metadata (page table, LRU,
//! free list) is protected by a brief Mutex held only during lookups.
...
/// Shared metadata protected by a single Mutex (held briefly).
struct Meta {
...
impl ConcurrentBufferPool {
    ...
    pub fn unpin_page(&self, page_id: PageId, is_dirty: bool) -> Result<()> {
        let mut meta = self.meta.lock().unwrap();
        let frame_idx = match meta.page_table.get(&page_id) {
            Some(&idx) => idx,
            None => {
    ...
    pub fn read_page(&self, page_id: PageId) -> Result<PageReadGuard<'_>> {
        ...
        drop(meta); // Release meta lock before acquiring page lock
    ...
    pub fn flush_all(&self) -> Result<()> {
        let meta = self.meta.lock().unwrap();
        let page_ids: Vec<(PageId, usize)> = meta.page_table.iter()
            .map(|(&pid, &idx)| (pid, idx))
            .collect();
        drop(meta);
    ...
    /// Find a free frame, evicting LRU if needed. Caller must hold meta lock.
